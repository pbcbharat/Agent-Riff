...
| Listen before playing | Human has played 4–8 notes | “Listen to that, then answer softly on violin.” | `tunein_get_session_state` or `tunein_listen`, then `tunein_perform_phrase` | Listen precedes perform; instrument is `violin`; phrase contains 1–32 valid note steps. |
| Stay for a live jam | Human asks for an ongoing call-and-response session | `tunein_listen`, `tunein_perform_phrase`, then `tunein_wait_for_human_phrase` after every agent reply | Agent remains in the task, responds after each completed human phrase, and stops honestly if the bounded wait times out. |
...
The target is at least 14/16 with no zero on **Listen before playing**, **Change direction**, or **Missing agent seat**.
